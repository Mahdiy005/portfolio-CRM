    ...
    <div id="myBtnContainer" class="mt-4 text-center">
        ...
        @if (count($categories) > 0)
            @foreach ($categories as $category)
                <button class="filter-item" data-filter="{{ str_replace(' ', '', $category->name)  }}">{{ $category->name }}</button>
            @endforeach
        @endif
    </div>
    <div class="portfolio-body">
        <div class="px-4 row justify-content-evenly">
            @if (count($workSectionData->works) > 0)
                ...
                <div class="mt-3 post col-md-4 All @foreach ($work->categories as $cat){{ str_replace(' ', '', $cat->name) . ' ' }} @endforeach col-10 mt-md-0" data-aos="fade-up">
                    <div class="card">
                        <img src="{{ asset("storage/projects/$work->image") }}" class="card-img-top" alt="..." />
